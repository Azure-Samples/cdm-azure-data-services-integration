...
class Base(object):
    ...
    def validate(self):
        ...
        for entry in self.schema:
            element = getattr(self, entry.name, tmp)
...
class MetadataObjectCollection(ObjectCollection):
    def __getitem__(self, index):
        if type(index) == str:
            index = next((i for i,item in enumerate(self) if item.name == index), None)
        if index is None:
            return None
...
class CsvFormatSettings(FileFormatSettings):
    schema = FileFormatSettings.schema + (
        ...
        SchemaEntry("encoding", int, 65001) # UTF-8 code page
    )
...
class Attribute(MetadataObject):
    schema = MetadataObject.schema + (
        SchemaEntry("dataCategory", String),
